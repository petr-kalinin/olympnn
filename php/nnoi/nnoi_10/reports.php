...
startonn("/Город/NNOI X/Результаты","Протокол тестирования X Нижегородской городской олимпиады школьников по информатике",array("/style.css","/ije_proc.css"));
?>
...
if (isset($_GET["prob"])and(isset($_GET["contestant"]))){
   ...
   if (isset($_GET["id"])and(isset($_GET["time"]))) {
      $time=(int)$_GET["time"];
      $id=(int)$_GET["id"];
      $fname=sprintf("reports/%s/%s_%s_%04d_%03d.xml",$prob,$boy,$prob,$time,$id);
